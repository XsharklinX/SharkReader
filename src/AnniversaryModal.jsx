...
export default function AnniversaryModal({ anniversaryInfo, onClose }) {
    const dialogRef = useModalA11y(!!anniversaryInfo, onClose);
    ...
    return (
        <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black/70 backdrop-blur-sm fade-in" onClick={onClose}>
            <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Aniversario de lectura" tabIndex={-1} className="bg-[var(--surface-bg)] rounded-3xl p-8 max-w-sm w-full shadow-2xl border border-[var(--border-color)] text-center mx-4 outline-none" onClick={e => e.stopPropagation()}>
